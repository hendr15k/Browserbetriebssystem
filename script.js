...
function openApp(appName, arg = null) {
    ...
    if (appName === 'terminal') {
        ...
        // Set a reasonable default size for memory game
        win.style.width = '450px';
        win.style.height = '550px';
        content = `
            <div class="memory-toolbar" style="padding: 10px; background: #eee; border-bottom: 1px solid #ccc; display: flex; justify-content: space-between; align-items: center;">
                <div style="font-weight: bold;">Time: <span id="mem-time-${windowId}">00:00</span></div>
                <div style="font-weight: bold;">Moves: <span id="mem-moves-${windowId}">0</span></div>
                <button onclick="initMemory('${windowId}')" style="padding: 5px 10px; cursor: pointer;">Restart</button>
            </div>
            <div id="mem-grid-${windowId}" class="memory-grid" style="padding: 10px; display: grid; grid-template-columns: repeat(4, 1fr); gap: 10px; height: 100%; align-content: center; justify-content: center; background: #2c3e50;">
...
function initMemory(windowId) {
    const grid = document.getElementById(`mem-grid-${windowId}`);
    const timeDisplay = document.getElementById(`mem-time-${windowId}`);
    const movesDisplay = document.getElementById(`mem-moves-${windowId}`);
    ...
    // Start Timer
    memoryGames[windowId].timerInterval = setInterval(() => {
        ...
        const currentTimerDisplay = document.getElementById(`mem-time-${windowId}`);
        if (currentTimerDisplay) {
            currentTimerDisplay.textContent = timeStr;
        }
    }, 1000);
}

function renderMemoryGrid(windowId) {
    const grid = document.getElementById(`mem-grid-${windowId}`);
    const game = memoryGames[windowId];
    if (!grid || !game) return;
    ...
    game.cards.forEach(card => {
        ...
        cardEl.className = `memory-card ${card.isFlipped || card.isMatched ? 'flipped' : ''}`;
        ...
        back.textContent = '❓'; // Or empty, or pattern

        cardEl.appendChild(front);
        cardEl.appendChild(back);

        cardEl.onclick = () => flipCard(windowId, card.id);

        grid.appendChild(cardEl);
    });
}

function flipCard(windowId, cardId) {
    const game = memoryGames[windowId];
    if (!game || game.lockBoard) return;
    ...
    // Update UI for this card specifically (optimization)
    const cardEl = document.querySelector(`#mem-grid-${windowId} .memory-card[data-id="${cardId}"]`);
    if (cardEl) cardEl.classList.add('flipped');

    if (game.flippedCards.length === 2) {
        game.moves++;
        document.getElementById(`mem-moves-${windowId}`).textContent = game.moves;
        checkForMatch(windowId);
    }
}

function checkForMatch(windowId) {
    const game = memoryGames[windowId];
    ...
    if (card1.emoji === card2.emoji) {
        ...
        if (game.matches === 8) {
            clearInterval(game.timerInterval);
            setTimeout(() => alert(`Congratulations! You won in ${game.moves} moves!`), 500);
        }
    } else {
        setTimeout(() => {
            ...
            // Update UI
            const el1 = document.querySelector(`#mem-grid-${windowId} .memory-card[data-id="${card1.id}"]`);
            const el2 = document.querySelector(`#mem-grid-${windowId} .memory-card[data-id="${card2.id}"]`);
