    ...
    if (appName === 'terminal') {
        ...
        content = `
            <div class="calendar-header">
                <button class="calendar-nav-btn" onclick="changeMonth('${windowId}', -1)">&#9664;</button>
                <div class="calendar-month-display" id="calendar-month-${windowId}">Month Year</div>
                <button class="calendar-nav-btn" onclick="changeMonth('${windowId}', 1)">&#9654;</button>
            </div>
            ...
            <div style="margin-top: 10px; text-align: center;">
                <button onclick="resetCalendar('${windowId}')" style="padding: 5px 10px; cursor: pointer; background: #3498db; color: white; border: none; border-radius: 3px;">Today</button>
            </div>
        `;
...
function closeWindow(windowId) {
    ...
    // Cleanup Calendar state if active
...
function changeMonth(windowId, offset) {
...
function resetCalendar(windowId) {
    const now = new Date();
    calendarStates[windowId] = {
        year: now.getFullYear(),
        month: now.getMonth()
...
function renderCalendar(windowId) {
    ...
    const monthDisplay = document.getElementById(`calendar-month-${windowId}`);
    if (!grid || !monthDisplay) return;

    // Set Month Title
    // Using German locale for months as requested implicitly by user language context, or stick to English to match UI?
    // The Clock tooltip is in German (memory). The user communicates in German.
    // The apps are in English (Notepad, File Explorer).
    // I will use 'default' locale which might use the browser's locale, or specify 'de-DE' if I want to force German.
    // Given the rest of the UI is English, I'll stick to English for consistency, or maybe localizable.
    // Let's use English to match "Notepad", "Terminal", etc.
    const monthName = new Date(state.year, state.month).toLocaleString('default', { month: 'long', year: 'numeric' });
    ...
    const firstDayOfMonth = new Date(state.year, state.month, 1).getDay(); // 0 (Sun) - 6 (Sat)
    ...
    for (let i = 0; i < firstDayOfMonth; i++) {
        ...
        div.textContent = prevMonthDays - firstDayOfMonth + 1 + i;
        grid.appendChild(div);
    }
    ...
    for (let i = 1; i <= daysInMonth; i++) {
        ...
        div.onclick = () => {
             // Maybe functionality to add event later?
             // For now just highlight selection visually or something.
        };

        grid.appendChild(div);
    }

    // Next month filler (to fill 6 rows, 42 cells total usually)
    const totalCells = firstDayOfMonth + daysInMonth;
    const remainingCells = (7 - (totalCells % 7)) % 7;
    // Better: Standard calendar usually has 5 or 6 rows.
    // Let's just fill until the grid looks square-ish or end of week.
    // Usually 6 rows * 7 = 42
