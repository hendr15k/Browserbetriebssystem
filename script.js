...
function toggleDesktop() {
    ...
    const anyVisible = Array.from(windows).some(w => w.style.display !== 'none');

    if (anyVisible) {
        // Minimize all visible
        windows.forEach(w => {
...
function openApp(appName, arg = null) {
    ...
    if (appName === 'terminal') {
        ...
        content = `
            <div class="video-player-content" style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; padding: 10px; background: #000; color: white; box-sizing: border-box;">
                <div id="video-track-name-${windowId}" style="margin-bottom: 10px; font-weight: bold; text-align: center; word-break: break-all;">No video selected</div>
                <video id="video-player-${windowId}" controls style="max-width: 100%; max-height: 80%; margin-bottom: 10px;"></video>
                <label style="background: #e91e63; color: white; padding: 8px 16px; border-radius: 4px; cursor: pointer; font-size: 14px;">
                    Open Video
        ...
        if (arg && fileSystem[arg]) {
             setTimeout(() => {
                 ...
                 const trackName = document.getElementById(`video-track-name-${windowId}`);
                 video.src = fileSystem[arg];
                 trackName.textContent = arg;
             }, 0);
        }
        ...
        content = `
            <div style="padding: 10px; background: white; height: 100%; box-sizing: border-box; display: flex; flex-direction: column;">
                <div style="display: flex; justify-content: space-between; margin-bottom: 10px; font-weight: bold; border-bottom: 2px solid #ccc; padding-bottom: 5px;">
                    <span>Application</span>
                    <span>CPU</span>
                    <span>Memory</span>
                    <span>Action</span>
                </div>
                <div id="tm-list-${windowId}" style="flex-grow: 1; overflow-y: auto;">
                    <!-- Tasks generated by JS -->
                </div>
                <div style="margin-top: 10px; border-top: 1px solid #ccc; padding-top: 5px; font-size: 12px; color: #666;">
                    Total Processes: <span id="tm-count-${windowId}">0</span>
                </div>
            </div>
        `;
        // Initialize Task Manager update loop
...
function renderFileExplorer(windowId) {
    ...
    Object.keys(fileSystem).forEach(key => {
        ...
        else if (displayName.endsWith('.mp4') || displayName.endsWith('.webm') || displayName.endsWith('.ogg')) iconChar = '🎥';
        ...
        fileDiv.onclick = () => {
             if (isDir) {
                 ...
                 } else if (displayName.endsWith('.mp4') || displayName.endsWith('.webm') || displayName.endsWith('.ogg')) {
                     openApp('video-player', key);
                 } else {
...
function initTaskManager(windowId) {
    ...
    taskManagerIntervals[windowId] = setInterval(() => updateTaskManager(windowId), 2000);
}

function updateTaskManager(windowId) {
    const list = document.getElementById(`tm-list-${windowId}`);
    const countSpan = document.getElementById(`tm-count-${windowId}`);
    if (!list) {
        clearInterval(taskManagerIntervals[windowId]);
        return;
    }
    ...
    list.innerHTML = '';
    const windows = document.querySelectorAll('.window');
    if (countSpan) countSpan.textContent = windows.length;
    ...
    windows.forEach(win => {
        ...
        if (titleEl) appTitle = titleEl.textContent;
        ...
        // Simulated stats
        const cpu = Math.floor(Math.random() * 15) + '%';
        const mem = Math.floor(Math.random() * 100 + 50) + ' MB';

        row.innerHTML = `
            <span style="width: 40%; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;">${appTitle}</span>
            <span style="width: 20%; text-align: right;">${cpu}</span>
            <span style="width: 20%; text-align: right;">${mem}</span>
            <div style="width: 20%; text-align: right;">
                 <button onclick="closeWindow('${winId}'); updateTaskManager('${windowId}')" style="background: #e74c3c; color: white; border: none; border-radius: 3px; cursor: pointer; padding: 2px 5px; font-size: 11px;">End</button>
            </div>
        `;
